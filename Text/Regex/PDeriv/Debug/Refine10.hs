{-# LANGUAGE GADTs, UnboxedTuples, DeriveGeneric #-}
...
module Text.Regex.PDeriv.Debug.Refine10 where
...
data RLevel = Strong | Weak deriving (Ord, Eq, Show, Generic)

instance Hashable RLevel
...
prune5' :: [(ChMap, Int, REnv)] -> [(ChMap, Int, REnv)]
prune5' ures = 
  let grouped = 
        foldl' (\m (cmap, state, renv) -> 
                case IM.lookup state m of 
                  { Nothing -> IM.insert state (cmap, state,renv) m 
                  ; Just (_,_,renv') -> case compareREnv renv renv' of
                    { LT -> IM.adjust (\_ ->  (cmap, state,renv)) state m 
                    ; _  -> m 
                    }
                  } ) IM.empty ures
  in map snd (IM.toList grouped)
...
urePDeriv (cmap, ur, r, psi) l = 
  let t = urPDeriv (ur, r) l Weak
  in t `seq` [ (cmap, ur', r''', psi'' `combineREnv` psi) | (ur', r', psi') <- t,  
...
redundantREnv renv cmap = False -- not helpful at all
...
urPDeriv :: (UReq, Re) -> Char -> RLevel -> [(UReq, Re, REnv)]
urPDeriv (ur, Eps i) l rlvl
  | i `inUR` ur = [ ((updateUR i r' ur), Eps i, singletonREnv i (singletonRS {- (RASt (Ch next_i l) Strong) -} (2000+(ord l)) )) 
                  | let r = fromJust (lookupUR i ur), r' <- pderiv r l ] 
  | otherwise   = [ (ur, Eps i, singletonREnv i (singletonRS {- (RASt (Ch next_i l) (maximal rlvl Weak)) -} (case rlvl of { Weak -> (3000+(ord l))  ; Strong -> (2000+(ord l)) })  )) ]
     
urPDeriv (ur, (Ch i l)) l' rlvl = 
  case lookup i ur of 
    { Just r | l == l' ->  [ ((updateUR i r' ur), (Eps i), singletonREnv i (singletonRS {- (RNoCh Strong) -} 6000 ))
                           | r' <- pderiv r l ]
      
             | l /= l' ->  [ ((updateUR i r' ur), (Eps i), singletonREnv i (singletonRS {- (RATr (Ch next_i2 l') Strong)-} (ord l'))) | r' <- pderiv r l]
    ; Nothing | l == l' -> [ (ur, Eps i, singletonREnv i (singletonRS $ {- RNoCh (maximal rlvl Weak) -} case rlvl of { Weak -> 7000 ; _ -> 6000 })) ]
              | l /= l' -> [ (ur, Eps i, singletonREnv i (singletonRS $ {- RATr (Ch next_i2 l') (maximal rlvl Weak) -} case rlvl of { Strong -> ord l' ; Weak -> 1000 + (ord l')}) ) ] 
    }

urPDeriv (ur, (Any i)) l rlvl =   
  case lookup i ur of 
    { Just r  -> [ ((updateUR i r' ur), (Eps i), singletonREnv i (singletonRS $ {- RNoCh Strong -} 6000) ) | r' <- pderiv r l ]
    ; Nothing -> [ (ur, Eps i, singletonREnv i (singletonRS $ {- RNoCh (maximal rlvl Weak) -} case rlvl of { Strong -> 6000 ; Weak -> 7000 }) ) ]
    }
    ...
    { Just p -> 
        case pderiv p l of
          ...
          ; ps  | posEmpty r1 ->  
               ...
               in t1 `seq` t2 `seq`  [ ((ur' ++ ur2 ++ [(i, Choice dontcare ps)]) , (Pair i r1' r2), renv) |  (ur', r1', renv) <- t1 ] ++ t2
                | otherwise   -> 
                 let ur2 =  ur `limit` fv r2 
                     t1 = urPDeriv (ur `limit` (fv r1), r1) l Strong 
                     t2 = urPDeriv (ur2, r2) l Strong
                 in t1 `seq` t2 `seq` [ ((ur' ++ ur2 ++ [(i, Choice dontcare ps)]) , (Pair i r1' r2), renv) |  (ur', r1', renv) <- t1] ++ [ (ur', r2', renv `combineREnv` renv') | (ur', r2', renv) <- t2, renv' <- mkFin r1 ]
          }
    ; Nothing | posEmpty r1 -> 
          let ur2 =  ur `limit` fv r2 
              t1  = urPDeriv (ur `limit` (fv r1), r1) l rlvl
              t2  = urPDeriv (ur2, r2) l rlvl 
          in t1 `seq` t2 `seq` [ ((ur' ++ ur2) , (Pair i r1' r2), renv) |  (ur', r1', renv) <-  t1 ] ++ t2
              | otherwise   -> 
          let ur2 =  ur `limit` fv r2
              t1  = urPDeriv (ur `limit` (fv r1), r1) l rlvl
              t2  = urPDeriv (ur2, r2) l rlvl 
          in t1 `seq` t2 `seq` [ ((ur' ++ ur `limit` (fv r2)) , (Pair i r1' r2), renv) |  (ur', r1', renv) <- t1 ] ++ [ (ur', r2', renv `combineREnv` renv') | (ur', r2', renv) <- t2, renv' <- mkFin r1 ]
    }
     ...
     { Just p ->  
         ...
         ; ps ->  
             let ur' = updateUR i (Choice dontcare ps) ur
                 ts = map (\ r -> urPDeriv (ur', r) l Strong) rs
             in  ts `seq` concat ts 
            -- todo:move i:is to each r
         }
     ; Nothing ->  
         let ts = map (\ r -> urPDeriv (ur, r) l rlvl) rs
         in ts `seq` concat ts
     }
    ...
    case lookup i ur of 
      { Just p -> 
         ...
         ; ps ->  
              ...
              in t `seq` [ (ur'', Pair i r' (Star i r), renv)  
                         | (ur'', r', renv) <- t ]
         }
      ; Nothing -> 
           let t = urPDeriv (ur,r) l rlvl
           in t `seq` [ (ur', Pair i r' (Star i r), renv)  
                      | (ur', r', renv) <- t ]
      }
...
-- mapping  (UReq,Re,Char,RLevel) -> [(UReq,Re,REnv)]
type TransTable = IM.IntMap (IM.IntMap [(Int,REnv)])
type FinalStates = IM.IntMap ()

-- mapping  (UReq,Re) -> ()
type AllStates = IM.IntMap ()
type Delta = [((UReq, Re), (Char, RLevel), [(UReq, Re, REnv)])]

buildTransTable :: (UReq, Re) -> (TransTable, FinalStates)
buildTransTable (ur, r) = 
  let sig                      = sigma (Any 0)
      emptyAllStates           = IM.empty
      emptyFinalStates         = IM.empty
      emptyDelta               = []
      (allstates,delta,finals) = go sig emptyAllStates emptyDelta emptyFinalStates [(ur,r)] 

  in (foldl' (\im (ur_r,l_lvl,ur_r_renvs) -> 
              let (ur,r) = ur_r
                  key1   = hash (ur,r)
                  (l,lvl) = l_lvl
                  key2   = hash (l,lvl)
                  i_renvs = map (\(ur,r,renv) -> (hash (ur,r), renv)) ur_r_renvs
              in case IM.lookup key1 im  of 
                { Just inner_im -> 
                     let inner_im' = case IM.lookup key2 inner_im of 
                           { Just i_renvs' -> IM.adjust (\_ -> i_renvs' ++ i_renvs) key2 inner_im
                           ; Nothing       -> IM.insert key2 i_renvs inner_im
                           }
                     in IM.adjust (\_ -> inner_im') key1 im
                ; Nothing -> let inner_im' = IM.singleton key2 i_renvs
                             in IM.insert key1 inner_im' im
                }                 
            ) IM.empty delta, finals)
    where go :: [Char] -> AllStates -> Delta -> FinalStates -> [(UReq, Re)] -> (AllStates, Delta, FinalStates) 
          -- invariant: currStates `intersect` allStates  == emptySet
          go sig allStates allDelta finals [] = (allStates,allDelta, finals)
          go sig allStates allDelta finals currStates = 
            let allStates' = allStates `IM.union` (IM.fromList (map (\x -> (hash x, ())) currStates))
                finals'    = finals `IM.union` (IM.fromList (map (\x -> (hash x, ())) (filter (\(ur,r) -> posEmpty r) currStates)))
                newDelta   = nub [ ur_r_renv `seq` (ur_r, (l,lvl), ur_r_renv') | ur_r <- currStates 
                                                                               , l      <- sig
                                                                               , lvl    <- [ Strong, Weak ]
                                                                               , let ur_r_renv = urPDeriv ur_r l lvl 
                                                                                     ur_r_renv' = map (\(ur,r,renv) ->  
                                                                                                        let (r',renv') = simpl r renv
                                                                                                        in (ur,r',renv')) ur_r_renv 
                                                                               ]
                newStates  = nub [ (ur,r) | (_,_,ur_r_renvs) <- newDelta
                                          , (ur,r,renv)      <- ur_r_renvs
                                          , (hash (ur,r)) `IM.notMember` allStates']
                
                allDelta'  = allDelta ++ newDelta
            in allStates' `seq` finals `seq` allDelta' `seq` newStates `seq`  go sig allStates' allDelta' finals' newStates 

-- tabled functions
turPDeriv :: TransTable -> Int -> Char -> RLevel -> [(Int, REnv)]
turPDeriv t state l lvl = case IM.lookup state t of
  { Just im -> case IM.lookup (hash (l,lvl)) im of 
       { Just i_renvs -> i_renvs 
       ; Nothing      -> [] 
       }
  ; Nothing         -> [] 
  }           
                           
turePDeriv :: TransTable -> (ChMap, Int, REnv) -> Char -> [(ChMap, Int, REnv)]
turePDeriv tbl (cmap, state, psi) l = 
  let t = turPDeriv tbl state l Weak
  in t `seq` [ (cmap, state', psi' `combineREnv` psi) | (state', psi') <- t,  
               not (redundantREnv psi' cmap) ]       
         


trefine :: UReq -> Re -> S.ByteString -> [Re]
trefine ureq r cs = 
  let cmap = choiceMap r 
      (tbl,finals)  = buildTransTable (ureq,r)
      s    = hash (ureq,r)
      renvs = nub $ sortBy compareREnv (tref (tbl,finals) [(cmap, s, emptyREnv)] cs)
   in 
     map (\renv ->  apply_ renv r)  renvs
...
tref :: (TransTable, FinalStates) -> [(ChMap, Int, REnv)] -> S.ByteString -> [REnv]
tref (tbl,finals) urs bs = case S.uncons bs of
  { Nothing ->
        [ renv | (cmap, st, renv) <- urs , st `IM.member` finals ] {- ++      
        [ renv' `combineREnv` renv   -- try to fix those states which are non-final?
        | (cmap, st, renv) <- urs
        , renv' <- mkFin r
        , not (posEmpty {- (renv `apply_` r) -} r)
        , any (\i -> case lookupUR i ureq of
                  ...
                  ; Just t  -> posEmpty t }) (getLabel r) ] -}
  ; Just (l,w) ->
         let urs' =  prune5' $ concatMap (\ (cmap, s,renv) -> turePDeriv tbl (cmap, s, renv) l) urs 
         in  {- io `seq` -} urs' `seq` tref (tbl,finals) urs' w
  }
...
test2 :: UReq -> String -> S.ByteString -> Maybe String
test2 g pat_s word = 
  case parse pat_s of
    Nothing -> Nothing
    Just r  -> Just $ pretty $ (trefine g r word) !! 0
